...
namespace Tetris
{
    class TetrisGame : Scene
    {
        ...
        Point cellSize = new Point(45, 45);
        ...
        public TetrisGame()
        {
            block = Global.content.Load<Texture2D>("WhiteBlock");
            border = Global.content.Load<Texture2D>("WhiteBorder");
        ...
        {
            //draw background
            sb.Draw(whiteBlock, new Rectangle(0, 0, Global.resolution.X, Global.resolution.Y), Color.White);


            var offset = new Point(Global.resolution.X / 2, 0) - cellSize * new Point(gridSize.X / 2, 0);
            //draw grid
            sb.Draw(whiteBlock, new Rectangle(offset.X, offset.Y, gridSize.X * cellSize.X, gridSize.Y * cellSize.Y), Color.Blue);
            ...
            for (var y = 0; y < tetromino[rotation].Length; y++)
                for (var x = 0; x < tetromino[rotation][y].Length; x++)
                    ...
                    {
                        var pos = (position + new Vector2(x, y)).ToPoint() * cellSize + offset;
                        sb.Draw(block, new Rectangle(pos, cellSize), Color.Black);
                    }

            //draw placed blocks
            foreach (var pos in placedBlocks)
                sb.Draw(block, new Rectangle(pos.ToPoint() * cellSize + offset, cellSize), Color.Black);
        }
